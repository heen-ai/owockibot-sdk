...
class OwockibotClient {
    constructor(config = {}) {
        ...
        this.http.interceptors.response.use((response) => response, (error) => {
            ...
            if (status === 404) {
                return Promise.reject(new types_1.NotFoundError(message, code));
            }
            else if (status === 400) {
                return Promise.reject(new types_1.ValidationError(message, code));
            }
            else if (status === 429) {
                return Promise.reject(new types_1.RateLimitError(message, code));
            }
            else if (status && status >= 500) {
                return Promise.reject(new types_1.ServerError(message, status, code));
            }
            else {
                // Generic API error for other status codes or unknown errors
                return Promise.reject(new types_1.ApiError(message, status, code));
            }
        });
    }
    ...
    async getBounties() {
        try {
            const response = await this.http.get('/bounty-board');
     ...
     */
    async getBountiesByStatus(status) {
        try {
            const bounties = await this.getBounties();
     ...
     */
    async searchBounties(keyword) {
        try {
            const bounties = await this.getBounties();
            const searchTerm = keyword.toLowerCase();
            return bounties.filter(b => b.title.toLowerCase().includes(searchTerm) ||
     ...
     */
    async getBountiesByCreator(creatorAddress) {
        try {
            const bounties = await this.getBounties();
            return bounties.filter(b => b.creator_address.toLowerCase() === creatorAddress.toLowerCase());
        }
        catch (error) {
            throw error;
        }
     ...
     */
    async getBountiesByClaimer(claimerAddress) {
        try {
            const bounties = await this.getBounties();
            return bounties.filter(b => b.claimer_address &&
    ...
    async getTreasuryStats() {
        try {
            const response = await this.http.get('/treasury');
            return response.data;
        }
        catch (error) {
    ...
    async getRatioData() {
        try {
            const response = await this.http.get('/ratio');
            return response.data;
        }
        catch (error) {
...
exports.OwockibotClient = OwockibotClient;
